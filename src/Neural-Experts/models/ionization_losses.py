...
class IonizationLoss(nn.Module):
    ...
    def forward(self, output_pred, data=None, dataset=None):
        if data is None:
            raise ValueError("IonizationLoss requires a data batch")
        ...
        if gt_vals is None:
            raise ValueError("IonizationLoss expected 'nonmnfld_val' when segmentation_mode=False")
