...
This workspace version also supports training Switch-NeRF as a single-target INR on the ionization volume dataset.

Train from `src/Switch-NeRF`:

```sh
python -m switch_nerf.train \
  --config switch_nerf/configs/switch_nerf/ionization.yaml \
  --device cuda
```

Override the target attribute when needed:
...
- `result/Ionization/Switch-NeRF/<exp_id>/checkpoints/*.pth`
- `result/Ionization/Switch-NeRF/<exp_id>/configs/config.yaml`

The exported `configs/config.yaml` is the `validate_prediction.py`-ready config paired with the saved checkpoint.
