from typing import Dict, List, Optional, Sequence, Tuple

import torch
import torch.nn as nn
import torch.nn.functional as F

from .moe_inr import ExpertDecoder, PolicyNetwork, SharedSirenEncoder2


class TimeMLP(nn.Module):
    def __init__(self, in_dim: int = 1, hidden_dim: int = 32, out_dim: int = 16):
        super().__init__()
        self.net = nn.Sequential(
            nn.Linear(in_dim, hidden_dim),
            nn.SiLU(),
            nn.Linear(hidden_dim, out_dim),
        )

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        return self.net(x)


def _as_tuple3(value) -> Tuple[int, int, int]:
    if isinstance(value, (list, tuple)) and len(value) == 3:
        return int(value[0]), int(value[1]), int(value[2])
    v = int(value)
    return v, v, v


def _compute_bbox(coords: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
    bbox_min = coords.min(dim=0).values
    bbox_max = coords.max(dim=0).values
    return bbox_min, bbox_max


def _compute_bin_stats(
    coords: torch.Tensor,
    values: torch.Tensor,
    grid_size: Tuple[int, int, int],
    bbox_min: torch.Tensor,
    bbox_max: torch.Tensor,
    eps: float,
) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    coords = coords[:, :3]
    values = values.view(values.shape[0], -1).mean(dim=-1)
    grid_size_t = torch.tensor(grid_size, device=coords.device, dtype=torch.float32)
    scale = (bbox_max - bbox_min).clamp_min(eps)
    normalized = (coords - bbox_min) / scale
    normalized = normalized.clamp(0.0, 1.0)
    idx = torch.floor(normalized * grid_size_t).to(torch.int64)
    idx = torch.minimum(idx, torch.tensor(grid_size, device=coords.device, dtype=torch.int64) - 1)

    flat_idx = idx[:, 0] * (grid_size[1] * grid_size[2]) + idx[:, 1] * grid_size[2] + idx[:, 2]
    total_bins = grid_size[0] * grid_size[1] * grid_size[2]

    grid_sum = torch.zeros(total_bins, device=coords.device, dtype=values.dtype)
    grid_cnt = torch.zeros(total_bins, device=coords.device, dtype=values.dtype)
    grid_sum_sq = torch.zeros(total_bins, device=coords.device, dtype=values.dtype)
    grid_sum.scatter_add_(0, flat_idx, values)
    grid_cnt.scatter_add_(0, flat_idx, torch.ones_like(values))
    grid_sum_sq.scatter_add_(0, flat_idx, values * values)

    grid_sum = grid_sum.view(*grid_size)
    grid_cnt = grid_cnt.view(*grid_size)
    grid_sum_sq = grid_sum_sq.view(*grid_size)
    return grid_sum, grid_cnt, grid_sum_sq


def _make_gaussian_kernel(size: int, sigma: Optional[float] = None, device=None, dtype=None) -> torch.Tensor:
    if sigma is None:
        sigma = float(size) / 3.0
    coords = torch.arange(size, device=device, dtype=dtype) - (size - 1) / 2.0
    kernel_1d = torch.exp(-0.5 * (coords / sigma) ** 2)
    kernel_1d = kernel_1d / kernel_1d.sum()
    kernel_3d = kernel_1d[:, None, None] * kernel_1d[None, :, None] * kernel_1d[None, None, :]
    return kernel_3d


def _smooth_grid(
    grid_sum: torch.Tensor, grid_cnt: torch.Tensor, kernel: torch.Tensor, eps: float
) -> torch.Tensor:
    grid_sum = grid_sum.unsqueeze(0).unsqueeze(0)
    grid_cnt = grid_cnt.unsqueeze(0).unsqueeze(0)
    kernel = kernel.unsqueeze(0).unsqueeze(0)
    pad = kernel.shape[-1] // 2
    smooth_sum = F.conv3d(grid_sum, kernel, padding=pad)
    smooth_cnt = F.conv3d(grid_cnt, kernel, padding=pad)
    smooth = smooth_sum / smooth_cnt.clamp_min(eps)
    return smooth.squeeze(0).squeeze(0)


def compute_indicator_grid(
    coords: torch.Tensor,
    values: torch.Tensor,
    grid_size: Tuple[int, int, int],
    kernel_sizes: Sequence[int],
    eps: float = 1e-6,
) -> Tuple[Dict[str, torch.Tensor], torch.Tensor, torch.Tensor]:
    bbox_min, bbox_max = _compute_bbox(coords[:, :3])
    grid_sum, grid_cnt, grid_sum_sq = _compute_bin_stats(coords, values, grid_size, bbox_min, bbox_max, eps)
    grid_cnt_safe = grid_cnt.clamp_min(1.0)
    grid_mean = grid_sum / grid_cnt_safe
    grid_var = grid_sum_sq / grid_cnt_safe - grid_mean * grid_mean

    smoothed = []
    for size in kernel_sizes:
        size = int(size)
        if size % 2 == 0:
            size += 1
        kernel = _make_gaussian_kernel(size, device=coords.device, dtype=coords.dtype)
        smoothed.append(_smooth_grid(grid_sum, grid_cnt, kernel, eps))

    if len(smoothed) == 0:
        diffs = [grid_mean.abs()]
    elif len(smoothed) == 1:
        diffs = [(grid_mean - smoothed[0]).abs()]
    else:
        diffs = [(smoothed[i - 1] - smoothed[i]).abs() for i in range(1, len(smoothed))]

    hf_energy = torch.zeros_like(grid_mean)
    for d in diffs:
        hf_energy = hf_energy + d

    def _normalize(t: torch.Tensor) -> torch.Tensor:
        min_val = t.min()
        max_val = t.max()
        return (t - min_val) / (max_val - min_val + eps)

    density = _normalize(grid_cnt)
    variance = _normalize(grid_var.clamp_min(0.0))
    hf_energy = _normalize(hf_energy)
    diffs = [_normalize(d) for d in diffs]

    channels: Dict[str, torch.Tensor] = {
        "hf": hf_energy,
        "density": density,
        "variance": variance,
        "scale_diffs": torch.stack(diffs, dim=-1) if len(diffs) > 1 else diffs[0].unsqueeze(-1),
    }
    return channels, bbox_min, bbox_max


class TimeConditionedIndicatorGrid(nn.Module):
    def __init__(
        self,
        grid_values: torch.Tensor,
        bbox_min: torch.Tensor,
        bbox_max: torch.Tensor,
        time_hidden_dim: int = 32,
        time_softmax: bool = True,
        time_weight_scale: float = 1.0,
        prior: Optional[torch.Tensor] = None,
    ):
        super().__init__()
        if grid_values.dim() != 5:
            raise ValueError("grid_values must have shape (K, X, Y, Z, C)")
        self.grid = nn.Parameter(grid_values)
        self.register_buffer("bbox_min", bbox_min)
        self.register_buffer("bbox_max", bbox_max)
        self.register_buffer("grid_size", torch.tensor(grid_values.shape[1:4], dtype=torch.int64))
        if prior is None:
            prior = grid_values.detach().clone()
        self.register_buffer("prior", prior)
        self.time_mlp = TimeMLP(in_dim=1, hidden_dim=int(time_hidden_dim), out_dim=grid_values.shape[0])
        self.time_softmax = bool(time_softmax)
        self.time_weight_scale = float(time_weight_scale)

    def _sample_bases(self, coords: torch.Tensor) -> torch.Tensor:
        coords = coords[:, :3]
        eps = 1e-6
        scale = (self.bbox_max - self.bbox_min).clamp_min(eps)
        normalized = (coords - self.bbox_min) / scale
        normalized = normalized.clamp(0.0, 1.0)

        grid_size = self.grid_size.to(coords.device).to(coords.dtype)
        pos = normalized * (grid_size - 1.0)
        idx0 = torch.floor(pos).to(torch.int64)
        idx1 = torch.minimum(idx0 + 1, (self.grid_size - 1).to(coords.device))
        weight = (pos - idx0.to(coords.dtype)).clamp(0.0, 1.0)

        x0, y0, z0 = idx0[:, 0], idx0[:, 1], idx0[:, 2]
        x1, y1, z1 = idx1[:, 0], idx1[:, 1], idx1[:, 2]
        wx, wy, wz = weight[:, 0:1], weight[:, 1:2], weight[:, 2:3]
        wx = wx.unsqueeze(1)
        wy = wy.unsqueeze(1)
        wz = wz.unsqueeze(1)

        c000 = self.grid[:, x0, y0, z0].permute(1, 0, 2)
        c100 = self.grid[:, x1, y0, z0].permute(1, 0, 2)
        c010 = self.grid[:, x0, y1, z0].permute(1, 0, 2)
        c110 = self.grid[:, x1, y1, z0].permute(1, 0, 2)
        c001 = self.grid[:, x0, y0, z1].permute(1, 0, 2)
        c101 = self.grid[:, x1, y0, z1].permute(1, 0, 2)
        c011 = self.grid[:, x0, y1, z1].permute(1, 0, 2)
        c111 = self.grid[:, x1, y1, z1].permute(1, 0, 2)

        c00 = c000 * (1.0 - wx) + c100 * wx
        c01 = c001 * (1.0 - wx) + c101 * wx
        c10 = c010 * (1.0 - wx) + c110 * wx
        c11 = c011 * (1.0 - wx) + c111 * wx
        c0 = c00 * (1.0 - wy) + c10 * wy
        c1 = c01 * (1.0 - wy) + c11 * wy
        return c0 * (1.0 - wz) + c1 * wz

    def forward(self, coords: torch.Tensor) -> torch.Tensor:
        feat_k = self._sample_bases(coords)
        if coords.shape[1] > 3:
            t = coords[:, -1:].contiguous()
        else:
            t = torch.zeros(coords.shape[0], 1, device=coords.device, dtype=coords.dtype)
        weights = self.time_mlp(t)
        if self.time_weight_scale != 1.0:
            weights = weights * self.time_weight_scale
        if self.time_softmax:
            weights = torch.softmax(weights, dim=-1)
        return (feat_k * weights.unsqueeze(-1)).sum(dim=1)

    def regularization(
        self,
        smooth_weight: float = 0.0,
        prior_weight: float = 0.0,
        amplitude_weight: float = 0.0,
        amplitude_min: float = 0.0,
        amplitude_max: float = 1.0,
        smooth_kernel: int = 3,
    ) -> torch.Tensor:
        reg = torch.zeros(1, device=self.grid.device, dtype=self.grid.dtype)
        if smooth_weight > 0.0:
            k = max(1, int(smooth_kernel))
            if k % 2 == 0:
                k += 1
            grid = self.grid.permute(0, 4, 1, 2, 3)
            smooth = F.avg_pool3d(grid, kernel_size=k, stride=1, padding=k // 2)
            reg = reg + smooth_weight * F.mse_loss(grid, smooth)
        if prior_weight > 0.0:
            reg = reg + prior_weight * F.mse_loss(self.grid, self.prior)
        if amplitude_weight > 0.0:
            hi = F.relu(self.grid - float(amplitude_max))
            lo = F.relu(float(amplitude_min) - self.grid)
            reg = reg + amplitude_weight * (hi * hi + lo * lo).mean()
        return reg


class ConditioningMoEINRHighLowFrequency(nn.Module):
    def __init__(
        self,
        in_features: int = 4,
        out_features: int = 1,
        num_experts: int = 7,
        num_low_freq_experts: Optional[int] = None,
        indicator_dim: int = 1,
        indicator_grid: Optional[TimeConditionedIndicatorGrid] = None,
        indicator_features: Optional[List[str]] = None,
        hf_indicator_index: Optional[int] = None,
        time_feature_dim: int = 0,
        time_mlp_hidden_dim: int = 32,
        indicator_reg_smooth: float = 0.0,
        indicator_reg_prior: float = 0.0,
        indicator_reg_amplitude: float = 0.0,
        indicator_reg_amplitude_min: float = 0.0,
        indicator_reg_amplitude_max: float = 1.0,
        indicator_reg_smooth_kernel: int = 3,
        hard_routing_at_eval: bool = True,
        straight_through_routing: bool = False,
        routing_top_k: int = 1,
        routing_load_balance_weight: float = 0.0,
        routing_group_split_weight: float = 0.0,
        routing_group_split_ratio: Optional[float] = None,
        routing_hf_group_weight: float = 1.0,
        routing_hf_center: float = 0.5,
        routing_hf_temperature: float = 0.2,
        routing_hf_min: float = 0.05,
        routing_hf_max: float = 0.95,
        routing_entropy_weight: float = 0.0,
        encoder_feature_dim: int = 256,
        encoder_first_omega_0: float = 30.0,
        encoder_hidden_omega_0: float = 30.0,
        policy_hidden_dim: int = 128,
        policy_num_layers: int = 3,
        policy_first_omega_0: float = 30.0,
        policy_hidden_omega_0: float = 30.0,
        high_expert_hidden_dim: int = 320,
        high_expert_num_layers: int = 4,
        high_expert_first_omega_0: float = 60.0,
        high_expert_hidden_omega_0: float = 60.0,
        low_expert_hidden_dim: int = 192,
        low_expert_num_layers: int = 3,
        low_expert_first_omega_0: float = 15.0,
        low_expert_hidden_omega_0: float = 15.0,
        low_freq_consistency_weight: float = 0.0,
        low_freq_consistency_std: float = 1.0e-3,
        low_freq_consistency_samples: int = 2048,
        low_freq_consistency_spatial_only: bool = True,
        input_mean: Optional[torch.Tensor] = None,
        input_std: Optional[torch.Tensor] = None,
    ):
        super().__init__()
        self.encoder = SharedSirenEncoder2(
            in_features=in_features,
            feature_dim=encoder_feature_dim,
            num_frequencies=6,
            include_input=True,
            first_omega_0=encoder_first_omega_0,
            hidden_omega_0=encoder_hidden_omega_0,
        )

        self.grid_indicator_dim = int(indicator_dim)
        self.time_feature_dim = max(0, int(time_feature_dim))
        self.time_mlp = None
        if self.time_feature_dim > 0:
            self.time_mlp = TimeMLP(in_dim=1, hidden_dim=int(time_mlp_hidden_dim), out_dim=self.time_feature_dim)

        total_indicator_dim = self.grid_indicator_dim + self.time_feature_dim
        policy_in_features = in_features + total_indicator_dim
        self.policy = PolicyNetwork(
            in_features=policy_in_features,
            hidden_dim=policy_hidden_dim,
            num_layers=policy_num_layers,
            num_experts=num_experts,
            first_omega_0=policy_first_omega_0,
            hidden_omega_0=policy_hidden_omega_0,
        )

        fused_dim = encoder_feature_dim + policy_hidden_dim + total_indicator_dim
        self.num_experts = int(num_experts)
        if num_low_freq_experts is None:
            num_low_freq_experts = max(1, self.num_experts // 2)
        self.num_low_freq_experts = int(num_low_freq_experts)
        self.num_low_freq_experts = min(self.num_low_freq_experts, self.num_experts)
        self.num_high_freq_experts = self.num_experts - self.num_low_freq_experts
        self.low_freq_indices = list(range(self.num_high_freq_experts, self.num_experts))

        experts: List[ExpertDecoder] = []
        for _ in range(self.num_high_freq_experts):
            experts.append(
                ExpertDecoder(
                    in_dim=fused_dim,
                    out_features=out_features,
                    hidden_dim=int(high_expert_hidden_dim),
                    num_layers=int(high_expert_num_layers),
                    first_omega_0=float(high_expert_first_omega_0),
                    hidden_omega_0=float(high_expert_hidden_omega_0),
                )
            )
        for _ in range(self.num_low_freq_experts):
            experts.append(
                ExpertDecoder(
                    in_dim=fused_dim,
                    out_features=out_features,
                    hidden_dim=int(low_expert_hidden_dim),
                    num_layers=int(low_expert_num_layers),
                    first_omega_0=float(low_expert_first_omega_0),
                    hidden_omega_0=float(low_expert_hidden_omega_0),
                )
            )
        self.experts = nn.ModuleList(experts)
        self.out_features = out_features
        self.indicator_grid = indicator_grid
        self.indicator_dim = total_indicator_dim
        self.indicator_features = indicator_features or ["hf"]
        self.hf_indicator_index = hf_indicator_index
        self.indicator_reg_smooth = float(indicator_reg_smooth)
        self.indicator_reg_prior = float(indicator_reg_prior)
        self.indicator_reg_amplitude = float(indicator_reg_amplitude)
        self.indicator_reg_amplitude_min = float(indicator_reg_amplitude_min)
        self.indicator_reg_amplitude_max = float(indicator_reg_amplitude_max)
        self.indicator_reg_smooth_kernel = int(indicator_reg_smooth_kernel)
        self.hard_routing_at_eval = bool(hard_routing_at_eval)
        self.straight_through_routing = bool(straight_through_routing)
        self.routing_top_k = max(1, int(routing_top_k))
        self.routing_load_balance_weight = float(routing_load_balance_weight)
        self.routing_group_split_weight = float(routing_group_split_weight)
        self.routing_group_split_ratio = routing_group_split_ratio
        self.routing_hf_group_weight = float(routing_hf_group_weight)
        self.routing_hf_center = float(routing_hf_center)
        self.routing_hf_temperature = float(routing_hf_temperature)
        self.routing_hf_min = float(routing_hf_min)
        self.routing_hf_max = float(routing_hf_max)
        self.routing_entropy_weight = float(routing_entropy_weight)
        self.low_freq_consistency_weight = float(low_freq_consistency_weight)
        self.low_freq_consistency_std = float(low_freq_consistency_std)
        self.low_freq_consistency_samples = int(low_freq_consistency_samples)
        self.low_freq_consistency_spatial_only = bool(low_freq_consistency_spatial_only)
        self._last_probs = None
        self._last_low_consistency = None

        self.register_buffer("input_mean", input_mean if input_mean is not None else torch.zeros(1, in_features))
        self.register_buffer("input_std", input_std if input_std is not None else torch.ones(1, in_features))
        self.use_input_stats = input_mean is not None and input_std is not None

    def _to_raw_coords(self, x: torch.Tensor) -> torch.Tensor:
        if not self.use_input_stats:
            return x
        return x * self.input_std.to(x.device) + self.input_mean.to(x.device)

    def indicator_regularization(self) -> torch.Tensor:
        if self.indicator_grid is None:
            return torch.zeros(1, device=self.input_mean.device, dtype=self.input_mean.dtype)
        return self.indicator_grid.regularization(
            smooth_weight=self.indicator_reg_smooth,
            prior_weight=self.indicator_reg_prior,
            amplitude_weight=self.indicator_reg_amplitude,
            amplitude_min=self.indicator_reg_amplitude_min,
            amplitude_max=self.indicator_reg_amplitude_max,
            smooth_kernel=self.indicator_reg_smooth_kernel,
        )

    def routing_regularization(self) -> torch.Tensor:
        if self._last_probs is None:
            return torch.zeros(1, device=self.input_mean.device, dtype=self.input_mean.dtype)
        probs = self._last_probs
        reg = torch.zeros(1, device=probs.device, dtype=probs.dtype)
        if self.routing_load_balance_weight > 0.0:
            mean_probs = probs.mean(dim=0)

            def _group_uniform_loss(indices: List[int]) -> torch.Tensor:
                if len(indices) == 0:
                    return torch.zeros(1, device=probs.device, dtype=probs.dtype)
                group_probs = mean_probs[indices]
                group_sum = group_probs.sum().clamp_min(1e-6)
                normalized = group_probs / group_sum
                target = torch.full_like(normalized, 1.0 / float(len(indices)))
                return torch.sum((normalized - target) ** 2)

            reg = reg + self.routing_load_balance_weight * (
                _group_uniform_loss(list(range(self.num_high_freq_experts)))
                + _group_uniform_loss(self.low_freq_indices)
            )
        if self.routing_group_split_weight > 0.0:
            mean_probs = probs.mean(dim=0)
            high_sum = mean_probs[: self.num_high_freq_experts].sum()
            target_high = float(max(0.0, min(1.0, target_high)))
            target_low = 1.0 - target_high
            low_sum = mean_probs[self.num_high_freq_experts :].sum()
            split_loss = (high_sum - target_high) ** 2 + (low_sum - target_low) ** 2
            reg = reg + self.routing_group_split_weight * split_loss
        if self.routing_entropy_weight > 0.0:
            entropy = -(probs * (probs + 1e-9).log()).sum(dim=-1).mean()
            reg = reg - self.routing_entropy_weight * entropy
        return reg

    def low_frequency_regularization(self) -> torch.Tensor:
        if self._last_low_consistency is None:
            return torch.zeros(1, device=self.input_mean.device, dtype=self.input_mean.dtype)
        return self._last_low_consistency

    def regularization_loss(self) -> torch.Tensor:
        return self.indicator_regularization() + self.routing_regularization() + self.low_frequency_regularization()

    def _compute_indicator_features(self, x: torch.Tensor) -> torch.Tensor:
        if self.indicator_grid is not None:
            raw_coords = self._to_raw_coords(x)
            grid_feat = self.indicator_grid(raw_coords)
        else:
            grid_feat = torch.zeros(x.shape[0], self.grid_indicator_dim, device=x.device, dtype=x.dtype)

        if self.time_mlp is not None:
            raw_coords = self._to_raw_coords(x)
            t_feat = self.time_mlp(raw_coords[:, -1:].contiguous())
        else:
            t_feat = torch.zeros(x.shape[0], self.time_feature_dim, device=x.device, dtype=x.dtype)
        return torch.cat([grid_feat, t_feat], dim=-1)

    def _compute_low_group_output(self, x: torch.Tensor) -> torch.Tensor:
        enc_feat = self.encoder(x)
        indicator = self._compute_indicator_features(x)
        router_in = torch.cat([x, indicator], dim=-1)
        probs, _, pol_feat = self.policy(router_in)
        probs = self._apply_hf_group_routing(probs, indicator)
        
        fused = torch.cat([enc_feat, pol_feat, indicator], dim=-1)

        if not self.low_freq_indices:
            return torch.zeros(x.shape[0], self.out_features, device=x.device, dtype=x.dtype)

        preds = [self.experts[i](fused) for i in self.low_freq_indices]
        preds_low = torch.stack(preds, dim=1)
        probs_low = probs[:, self.low_freq_indices]
        return torch.sum(preds_low * probs_low.unsqueeze(-1), dim=1)

    def _apply_hf_group_routing(self, probs: torch.Tensor, indicator: torch.Tensor) -> torch.Tensor:
        if (
            self.routing_hf_group_weight <= 0.0
            or self.hf_indicator_index is None
            or self.num_high_freq_experts <= 0
            or self.num_low_freq_experts <= 0
        ):
            return probs
        hf = indicator[:, self.hf_indicator_index]
        temp = max(1e-6, self.routing_hf_temperature)
        gate = torch.sigmoid((hf - self.routing_hf_center) / temp)
        if self.routing_hf_min > 0.0 or self.routing_hf_max < 1.0:
            gate = gate.clamp(min=self.routing_hf_min, max=self.routing_hf_max)
        if self.routing_hf_group_weight != 1.0:
            gate = gate * self.routing_hf_group_weight + (1.0 - self.routing_hf_group_weight) * 0.5

        probs_high = probs[:, : self.num_high_freq_experts]
        probs_low = probs[:, self.num_high_freq_experts :]
        probs_high = probs_high / probs_high.sum(dim=-1, keepdim=True).clamp_min(1e-6)
        probs_low = probs_low / probs_low.sum(dim=-1, keepdim=True).clamp_min(1e-6)

        gate = gate.unsqueeze(-1)
        probs_high = probs_high * gate
        probs_low = probs_low * (1.0 - gate)
        return torch.cat([probs_high, probs_low], dim=-1)

    def forward(
        self,
        x: torch.Tensor,
        spatial_neighbor_index: torch.Tensor = None,
        temporal_neighbor_index: torch.Tensor = None,
        *,
        hard_routing: bool = False,
        return_all: bool = False,
    ):
        x_in = x

        enc_feat = self.encoder(x_in)
        indicator = self._compute_indicator_features(x_in)
        router_in = torch.cat([x_in, indicator], dim=-1)
        probs, logits, pol_feat = self.policy(router_in)
        probs = self._apply_hf_group_routing(probs, indicator)
        
        self._last_probs = probs
        fused = torch.cat([enc_feat, pol_feat, indicator], dim=-1)

        preds = [expert(fused) for expert in self.experts]
        preds_all = torch.stack(preds, dim=1)

        if self.training and self.low_freq_consistency_weight > 0.0 and self.low_freq_indices:
            preds_low = preds_all[:, self.low_freq_indices, :]
            probs_low = probs[:, self.low_freq_indices]
            y_low_full = torch.sum(preds_low * probs_low.unsqueeze(-1), dim=1)

            if self.low_freq_consistency_samples <= 0:
                self._last_low_consistency = torch.zeros(1, device=x_in.device, dtype=x_in.dtype)
            else:
                sample_count = min(y_low_full.shape[0], self.low_freq_consistency_samples)
                if sample_count <= 0:
                    self._last_low_consistency = torch.zeros(1, device=x_in.device, dtype=x_in.dtype)
                else:
                    perm = torch.randperm(y_low_full.shape[0], device=y_low_full.device)[:sample_count]
                    y_low = y_low_full[perm]
                    x_sub = x_in[perm]
                    x_pert = x_sub.clone()
                    if self.low_freq_consistency_spatial_only and x_pert.shape[-1] >= 3:
                        delta = torch.randn_like(x_pert[:, :3]) * self.low_freq_consistency_std
                        x_pert[:, :3] = x_pert[:, :3] + delta
                    else:
                        delta = torch.randn_like(x_pert) * self.low_freq_consistency_std
                        x_pert = x_pert + delta
                    y_low_pert = self._compute_low_group_output(x_pert)
                    self._last_low_consistency = self.low_freq_consistency_weight * F.mse_loss(y_low_pert, y_low)
        else:
            self._last_low_consistency = torch.zeros(1, device=x_in.device, dtype=x_in.dtype)

        use_hard = hard_routing or (not self.training and self.hard_routing_at_eval)
        if use_hard:
            if self.routing_top_k > 1:
                topk = torch.topk(probs, k=self.routing_top_k, dim=-1)
                mask = torch.zeros_like(probs).scatter_(1, topk.indices, 1.0)
                hard_probs = probs * mask
                hard_probs = hard_probs / hard_probs.sum(dim=-1, keepdim=True).clamp_min(1e-6)
            else:
                indices = torch.argmax(probs, dim=-1)
                hard_probs = F.one_hot(indices, num_classes=probs.shape[-1]).to(probs.dtype)

            if self.training and self.straight_through_routing:
                probs = hard_probs.detach() - probs.detach() + probs
            else:
                probs = hard_probs
        y = torch.sum(preds_all * probs.unsqueeze(-1), dim=1)

        if return_all:
            return y, preds_all, probs, logits, indicator
        return y


def build_conditioning_moe_inr_high_low_frequency_from_config(cfg, dataset=None) -> ConditioningMoEINRHighLowFrequency:
    indicator_dim = int(cfg.get("indicator_dim", 1))
    grid_size = _as_tuple3(cfg.get("indicator_grid_size", 16))
    kernel_sizes = cfg.get("indicator_kernel_sizes", [3, 7, 11])
    kernel_sizes = [int(k) for k in kernel_sizes]
    indicator_features = cfg.get("indicator_features", ["hf", "density", "variance", "scale_diffs"])
    hf_indicator_index = None

    def _infer_hf_index(feature_list: Sequence[str], kernel_sizes_local: Sequence[int]) -> Optional[int]:
        idx = 0
        for name in feature_list:
            if name == "scale_diffs":
                diffs_count = max(1, len(kernel_sizes_local) - 1)
                idx += diffs_count
                continue
            if name == "hf":
                return idx
            idx += 1
        return None
    basis_count = max(1, int(cfg.get("indicator_basis_count", cfg.get("indicator_rank", 8))))

    indicator_grid = None
    input_mean = None
    input_std = None

    if dataset is not None:
        x_raw = dataset.x
        y_raw = dataset.y
        if dataset.normalize:
            input_mean = dataset.x_mean
            input_std = dataset.x_std
            x_raw = x_raw * dataset.x_std + dataset.x_mean
            y_raw = y_raw * dataset.y_std + dataset.y_mean
        indicator_channels, bbox_min, bbox_max = compute_indicator_grid(
            x_raw.to(torch.float32),
            y_raw.to(torch.float32),
            grid_size=grid_size,
            kernel_sizes=kernel_sizes,
        )
        channel_list: List[torch.Tensor] = []
        running_index = 0
        for name in indicator_features:
            if name == "scale_diffs":
                scale_tensor = indicator_channels[name]
                if scale_tensor.dim() == 3:
                    scale_tensor = scale_tensor.unsqueeze(-1)
                for i in range(scale_tensor.shape[-1]):
                    channel_list.append(scale_tensor[..., i])
                    running_index += 1
            else:
                channel_list.append(indicator_channels[name])
                if name == "hf":
                    hf_indicator_index = running_index
                running_index += 1
        if len(channel_list) == 0:
            channel_list = [indicator_channels["hf"]]
        indicator_init = torch.stack(channel_list, dim=-1)
        if indicator_dim != indicator_init.shape[-1]:
            if indicator_dim < indicator_init.shape[-1]:
                indicator_init = indicator_init[..., :indicator_dim]
            else:
                pad = indicator_dim - indicator_init.shape[-1]
                indicator_init = torch.cat(
                    [indicator_init, torch.zeros(*indicator_init.shape[:-1], pad, device=indicator_init.device, dtype=indicator_init.dtype)],
                    dim=-1,
                )
        grid_init = torch.zeros((basis_count, *indicator_init.shape), device=indicator_init.device, dtype=indicator_init.dtype)
        grid_init[0] = indicator_init
        indicator_grid = TimeConditionedIndicatorGrid(
            grid_init,
            bbox_min,
            bbox_max,
            time_hidden_dim=int(cfg.get("indicator_time_mlp_hidden_dim", cfg.get("time_mlp_hidden_dim", 32))),
            time_softmax=bool(cfg.get("indicator_time_softmax", True)),
            time_weight_scale=float(cfg.get("indicator_time_weight_scale", 1.0)),
            prior=grid_init.clone(),
        )
    else:
        bbox_min = torch.tensor(cfg.get("indicator_bbox_min", [0.0, 0.0, 0.0]), dtype=torch.float32)
        bbox_max = torch.tensor(cfg.get("indicator_bbox_max", [1.0, 1.0, 1.0]), dtype=torch.float32)
        indicator_init = torch.zeros((*grid_size, indicator_dim), dtype=torch.float32)
        grid_init = torch.zeros((basis_count, *indicator_init.shape), dtype=indicator_init.dtype)
        grid_init[0] = indicator_init
        if hf_indicator_index is None:
            hf_indicator_index = _infer_hf_index(indicator_features, kernel_sizes)
        if hf_indicator_index is None and indicator_dim > 0:
            hf_indicator_index = 0
        indicator_grid = TimeConditionedIndicatorGrid(
            grid_init,
            bbox_min,
            bbox_max,
            time_hidden_dim=int(cfg.get("indicator_time_mlp_hidden_dim", cfg.get("time_mlp_hidden_dim", 32))),
            time_softmax=bool(cfg.get("indicator_time_softmax", True)),
            time_weight_scale=float(cfg.get("indicator_time_weight_scale", 1.0)),
            prior=grid_init.clone(),
        )

    expert_first = float(cfg.get("expert_first_omega_0", 30.0))
    expert_hidden = float(cfg.get("expert_hidden_omega_0", 30.0))
    expert_hidden_dim = int(cfg.get("expert_hidden_dim", 256))
    expert_num_layers = int(cfg.get("expert_num_layers", 3))

    high_first = float(cfg.get("high_expert_first_omega_0", max(60.0, expert_first * 2.0)))
    high_hidden = float(cfg.get("high_expert_hidden_omega_0", max(60.0, expert_hidden * 2.0)))
    high_dim = int(cfg.get("high_expert_hidden_dim", max(128, int(expert_hidden_dim * 1.5))))
    high_layers = int(cfg.get("high_expert_num_layers", max(3, expert_num_layers + 1)))

    low_first = float(cfg.get("low_expert_first_omega_0", min(15.0, expert_first * 0.5)))
    low_hidden = float(cfg.get("low_expert_hidden_omega_0", min(15.0, expert_hidden * 0.5)))
    low_dim = int(cfg.get("low_expert_hidden_dim", max(64, int(expert_hidden_dim * 0.75))))
    low_layers = int(cfg.get("low_expert_num_layers", max(2, expert_num_layers)))

    return ConditioningMoEINRHighLowFrequency(
        in_features=int(cfg.get("in_features", 4)),
        out_features=int(cfg.get("out_features", 1)),
        num_experts=int(cfg.get("num_experts", 7)),
        num_low_freq_experts=cfg.get("num_low_freq_experts"),
        indicator_dim=indicator_dim,
        indicator_grid=indicator_grid,
        indicator_features=indicator_features,
        hf_indicator_index=hf_indicator_index,
        time_feature_dim=int(cfg.get("time_feature_dim", 0)),
        time_mlp_hidden_dim=int(cfg.get("time_mlp_hidden_dim", 32)),
        indicator_reg_smooth=float(cfg.get("indicator_reg_smooth", 0.0)),
        indicator_reg_prior=float(cfg.get("indicator_reg_prior", 0.0)),
        indicator_reg_amplitude=float(cfg.get("indicator_reg_amplitude", 0.0)),
        indicator_reg_amplitude_min=float(cfg.get("indicator_reg_amplitude_min", 0.0)),
        indicator_reg_amplitude_max=float(cfg.get("indicator_reg_amplitude_max", 1.0)),
        indicator_reg_smooth_kernel=int(cfg.get("indicator_reg_smooth_kernel", 3)),
        hard_routing_at_eval=bool(cfg.get("hard_routing_at_eval", True)),
        straight_through_routing=bool(cfg.get("straight_through_routing", False)),
        routing_top_k=int(cfg.get("routing_top_k", 1)),
        routing_load_balance_weight=float(cfg.get("routing_load_balance_weight", 0.0)),
        routing_group_split_weight=float(cfg.get("routing_group_split_weight", cfg.get("routing_split_weight", 0.0))),
        routing_group_split_ratio=cfg.get("routing_group_split_ratio", cfg.get("routing_split_ratio")),
        routing_hf_group_weight=float(cfg.get("routing_hf_group_weight", 1.0)),
        routing_hf_center=float(cfg.get("routing_hf_center", 0.5)),
        routing_hf_temperature=float(cfg.get("routing_hf_temperature", 0.2)),
        routing_hf_min=float(cfg.get("routing_hf_min", 0.05)),
        routing_hf_max=float(cfg.get("routing_hf_max", 0.95)),
        routing_entropy_weight=float(cfg.get("routing_entropy_weight", 0.0)),
        encoder_feature_dim=int(cfg.get("encoder_feature_dim", 256)),
        encoder_first_omega_0=float(cfg.get("encoder_first_omega_0", 30.0)),
        encoder_hidden_omega_0=float(cfg.get("encoder_hidden_omega_0", 30.0)),
        policy_hidden_dim=int(cfg.get("policy_hidden_dim", 128)),
        policy_num_layers=int(cfg.get("policy_num_layers", 3)),
        policy_first_omega_0=float(cfg.get("policy_first_omega_0", 30.0)),
        policy_hidden_omega_0=float(cfg.get("policy_hidden_omega_0", 30.0)),
        high_expert_hidden_dim=high_dim,
        high_expert_num_layers=high_layers,
        high_expert_first_omega_0=high_first,
        high_expert_hidden_omega_0=high_hidden,
        low_expert_hidden_dim=low_dim,
        low_expert_num_layers=low_layers,
        low_expert_first_omega_0=low_first,
        low_expert_hidden_omega_0=low_hidden,
        low_freq_consistency_weight=float(cfg.get("low_freq_consistency_weight", cfg.get("low_freq_grad_weight", 0.0))),
        low_freq_consistency_std=float(cfg.get("low_freq_consistency_std", 1.0e-3)),
        low_freq_consistency_samples=int(cfg.get("low_freq_consistency_samples", 2048)),
        low_freq_consistency_spatial_only=bool(cfg.get("low_freq_consistency_spatial_only", cfg.get("low_freq_grad_spatial_only", True))),
        input_mean=input_mean,
        input_std=input_std,
    )
