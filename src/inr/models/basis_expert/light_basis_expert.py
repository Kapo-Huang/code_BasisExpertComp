...
class LightBasisExpert(nn.Module):
    ...
    def pretrain_teacher_shared_feat(self, coords: torch.Tensor, teacher_mode: str = "random_topk"):
        ...
        if mode == "uniform":
            ...
            topk = max(1, min(self.top_k, self.num_experts))
            indices = torch.randint(0, self.num_experts, (expert_feats.shape[0], topk), device=expert_feats.device)
            weights.scatter_(1, indices, 1.0 / float(topk))
        else:
